...
void colorizer(int nimg, float *network, float *inputs, float *outputs) {
	...
	MPI_Request request_network, request_inputs;
	MPI_Status status_network, status_inputs;

	MPI_Iscatter(inputs, num * 224 * 224, MPI_FLOAT, inputs, num * 224 * 224, MPI_FLOAT, 0, MPI_COMM_WORLD, &request_inputs);
	MPI_Ibcast(network, 43218402, MPI_FLOAT, 0, MPI_COMM_WORLD, &request_network);

	MPI_Wait(&request_inputs, &status_inputs);
	MPI_Wait(&request_network, &status_network);
	...
	int num2 = (rank + 1) * num >= nimg ? nimg - rank * num  : num;
...
//			printf("sigmoid output\n%lf ms\n", 1000 * (end - start));
	...
	MPI_Gather(outputs, num * 2 * 112 * 112, MPI_FLOAT, outputs, num * 2 * 112 * 112, MPI_FLOAT, 0, MPI_COMM_WORLD);
}
